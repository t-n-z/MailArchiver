    ...
    internal class NDB
    {
        ...
        private byte[] ReadAndDecompress(DataRef rb, out int read, byte[] buffer = null, int offset = 0)
        {
            ...
            lock (XstFile.StreamLock)
            {
                ...
                {
                    // LOCAL PATCH (Email Archiver): bound the decompressor to this block's
                    // compressed length. Upstream wrapped DeflateStream around the raw file
                    // stream with NO length limit, so on a 4K-format OST inflate reads far
                    // past the block — through huge swaths of the file — taking minutes per
                    // message and looking like a hang. A compressed block is exactly
                    // rb.Length bytes at rb.Offset: a 2-byte zlib header, then the raw
                    // deflate payload (the 4-byte Adler-32 trailer is ignored by
                    // DeflateStream, which stops at the deflate end-of-block marker).
                    if (rb.Length < 2)
                        throw new XstException("Compressed data block too small");

                    fs.Seek((long)rb.Offset, SeekOrigin.Begin);

                    byte[] compressedBlock = new byte[rb.Length];
                    fs.ReadExactly(compressedBlock, 0, rb.Length);

                    // The first two bytes are a zlib header which DeflateStream does not
                    // understand. They should be 0x789c (default-compression magic).
                    if (compressedBlock[0] != 0x78 || compressedBlock[1] != 0x9c)
                        throw new XstException("Unexpected header in compressed data stream");

                    if (buffer == null)
                        buffer = new byte[rb.InflatedLength];

                    using (var compressedStream = new MemoryStream(compressedBlock, 2, rb.Length - 2, writable: false))
